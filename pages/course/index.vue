...
export default {
  ...
  methods: {
    // 查询第一页课程
    initCourseAll() {
      courseApi.getPageTeacher(1, 8, this.searchObj).then((response) => {
    ...
    searchOne(id ,index) {
      ...
      // 点击一级分类，封装一级分类进查询条件，并进行查询
      this.searchObj.subjectParentId = id; 
      this.gotoPage(this.page);

      this.subSubjectList = [];

      // 此时点击的是全部按钮，直接跳转course页面
      if(id === ''){
        this.searchObj = {};
        this.gotoPage(this.page);
      }else{
        ...
        for(let i = 0;i < this.subjectNestedList.length;i++) {
          if(id === this.subjectNestedList[i].id) {
            // 将一级分类下的所有的二级分类List赋值给subSubjectList
    ...
    // 查询所有分类
    initSubjectAll() {
      courseApi.getSubjectAll().then((response) => {
    ...
    gotoPage(page) {
      ...
      courseApi.getPageTeacher(page, 8, this.searchObj).then((response) => {
        this.data = response.data.data;
      });
    },
    // 点击销量进行排序
    searchBuyCount() {
      // 将销量赋值，只需不等于空就行，此时需要清空其他的排序
      this.buyCountSort = "1";
      this.gmtCreateSort = "";
      this.priceSort = "";
